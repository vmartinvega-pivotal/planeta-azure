...
ALTER TABLE os.ext_connection ADD CONSTRAINT ext_check_port CHECK ((port > 0 AND type = 'oracle'::text) OR (port > 0 AND type = 'greenplum'::text) OR (type = 'sqlserver'::text AND port IS NULL) OR (type = 'azure'::text AND port > 0));
